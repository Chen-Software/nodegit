...
namespace nodegit {
  ...
  template<typename T, typename std::enable_if<std::is_same<T, long>::value && !std::is_same<long, int64_t>::value, int>::type = 0>
  inline Napi::Value ToV8(Napi::Env env, T value) { return Napi::Number::New(env, static_cast<double>(value)); }
  template<typename T, typename std::enable_if<std::is_same<T, unsigned long>::value && !std::is_same<unsigned long, uint64_t>::value, int>::type = 0>
  inline Napi::Value ToV8(Napi::Env env, T value) { return Napi::Number::New(env, static_cast<double>(value)); }
